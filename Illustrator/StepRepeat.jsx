...
//Contact: https://www.upwork.com/fl/olegbutrin
...
function UI (doc) {
	...
	var pnl_settings = mainframe.add('panel {orientation: "column", alignChild: ["fill", "fill"], alignment: "fill"}');

	var grp_repeat = pnl_settings.add('group {orientation: "row", alignChild: ["fill", "center"]}');
	var lbl_repeat = grp_repeat.add('statictext');
	var scb_repeat = grp_repeat.add('scrollbar {stepdelta: 1, value: 1, minvalue: 1, maxvalue: ' + _max_repeat + '}');
	var txt_repeat = grp_repeat.add('edittext');
	var sld_repeat = grp_repeat.add('slider {value: 1, minvalue: 1, maxvalue: ' + _max_repeat + '}');
	lbl_repeat.text = lang.repeat;

	var grp_stepx = pnl_settings.add('group {orientation: "row", alignChild: ["fill", "center"]}');
	var lbl_stepx = grp_stepx.add('statictext');
	var scb_stepx = grp_stepx.add('scrollbar {stepdelta: 1, value: 0, minvalue: 0, maxvalue: ' + (this.maxstep * 2) + '}');
	var txt_stepx = grp_stepx.add('edittext');
	var sld_stepx = grp_stepx.add('slider {value: 0, minvalue: 0, maxvalue: ' + (this.maxstep * 2) + '}');
	lbl_stepx.text = lang.steph;

	var grp_stepy = pnl_settings.add('group {orientation: "row", alignChild: ["fill", "center"]}');
	var lbl_stepy = grp_stepy.add('statictext');
	var scb_stepy = grp_stepy.add('scrollbar {stepdelta: 1, value: 0, minvalue: 0, maxvalue: ' + (this.maxstep * 2) + '}');
	var txt_stepy = grp_stepy.add('edittext');
	var sld_stepy = grp_stepy.add('slider {value: 0, minvalue: 0, maxvalue: ' + (this.maxstep * 2) + '}');
	lbl_stepy.text = lang.stepv;

	var grp_rotate = pnl_settings.add('group {orientation: "row", alignChild: ["fill", "center"]}');
	var lbl_rotate = grp_rotate.add('statictext');
	var scb_rotate = grp_rotate.add('scrollbar {stepdelta: 1, value: 0, minvalue: 0, maxvalue: 360}');
	var txt_rotate = grp_rotate.add('edittext');
	var sld_rotate = grp_rotate.add('slider {value: 0, minvalue: 0, maxvalue: 360}');
	...
	sld_repeat.onChange = function () {this.value = Math.floor(this.value); txt_repeat.text = this.value.toString(); scb_repeat.value = this.value;}
	scb_repeat.onChange = function () {this.value = Math.floor(this.value); txt_repeat.text = this.value.toString(); sld_repeat.value = this.value;}
	sld_repeat.value = 1;
	sld_repeat.notify();
	...
	sld_stepx.onChange = function () {this.value = parseFloat2(this.value); txt_stepx.text = parseFloat2(this.value - po.maxstep) + ' ' + po.unit; scb_stepx.value = this.value}
	scb_stepx.onChange = function () {this.value = parseFloat2(this.value); txt_stepx.text = parseFloat2(this.value - po.maxstep) + ' ' + po.unit; sld_stepx.value = this.value}
	...
	sld_stepy.onChange = function () { this.value = parseFloat2(this.value); txt_stepy.text = parseFloat2(this.value - po.maxstep) + ' ' + po.unit; scb_stepy.value = this.value};
	scb_stepy.onChange = function () { this.value = parseFloat2(this.value); txt_stepy.text = parseFloat2(this.value - po.maxstep) + ' ' + po.unit; sld_stepy.value = this.value};
	...
	sld_rotate.onChange = function () { this.value = parseFloat2(this.value); txt_rotate.text = parseFloat2(this.value - 180) + ' \u00B0'; scb_rotate.value = this.value}
	scb_rotate.onChange = function () { this.value = parseFloat2(this.value); txt_rotate.text = parseFloat2(this.value - 180) + ' \u00B0'; sld_rotate.value = this.value}
	sld_rotate.value = 180;
	sld_rotate.notify();
	...
	var btn_run = grp_buttons.add('button');
	var btn_cancel = grp_buttons.add('button');
	btn_run.text = lang.run;
	btn_cancel.text = lang.cancel;

	btn_run.onClick = function () {
		po.repeat = sld_repeat.value;
		po.stepx = parseFloat2(sld_stepx.value - po.maxstep);
		po.stepy = parseFloat2(sld_stepy.value - po.maxstep);
	...
	var lbl_copyright = pnl_copyright.add('statictext {justify: "center"}');
	var txt_copyright = pnl_copyright.add('edittext { properties: {multiline: false, readonly: true, borderless: true}, justify: "center"}');
	...
	this.dialog.addEventListener('keydown', function () {var state = ScriptUI.environment.keyboardState; if (state.keyName == 'Enter') {btn_run.active = true; btn_run.notify('onClick')}});
...
function main () {
	...
	for (var i = 0; i < ui.repeat; i++) {
		...
		item.translate(ui.stepx, -ui.stepy);
